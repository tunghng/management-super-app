...
RETURNS void AS '
    DECLARE
       row record;
    BEGIN
       IF (SELECT COUNT(*) FROM public.document) > (SELECT COUNT(*) FROM public.document_code) THEN
           FOR row IN SELECT * FROM public.document ORDER BY created_at LOOP

               INSERT INTO public.document_code (id)
                   VALUES (LPAD(NEXTVAL(''code_seq'')::TEXT, 7, ''0''));

               UPDATE public.document
                   SET document_code = LPAD(CURRVAL(''code_seq'')::TEXT, 7, ''0'')
                       WHERE id = row.id AND document_code IS NULL;
